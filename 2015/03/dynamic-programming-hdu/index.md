...
由于要计算逃脱概率，我们可以在读入的时候就把可能被逮捕的概率 $P$ 变成可能逃脱的概率 $1 - P$。这样，状态转移方程为：$$ f[j] = \max{\left(f[j - pValue[i]] \times pCost[i], f[j]\right)}$$  其中 $pValue[i]$ 表示第 $i$ 个银行的价值，$pCost[i]$ 表示偷第 $i$ 个银行逃脱的概率。

这时候，我们还需要考虑一个转移条件，如果 $f[j - pValue[i]]$ 没有被更新过，那么是不能转移过来的。因为暂时没有一个状态可以偷到 $j - pValue[i]$ 价值的物品。
...
输入两位小数的时候，把它乘以 100，做 0/1 背包就可以了，状态转移方程为：$$f[j] = \max{\left(f[j - pValue[i]] + pCost[i], f[j]\right)}$$ 数据方面可能输入的时候没有给出小数部分，比如直接输入 100，而不是 100.00。但是题目中没有给出报销限额的数据范围，开到 3000000 就可以了。
...
这道题目有一种贪心的做法，扫一遍 $pData[i]$ 求和（`Sum += pData[i]`），如果遇到当前和 $Sum$ 小于 0 的时候，就更新答案，并且把当前累加的和置为当前遍历到的数字 $pData[i]$。

还会有一种动态规划的方法：$$f[i] = \max{\left(f[i - 1] + pData[i], pData[i]\right)}$$ 每次更新以后都要更新答案。需要注意，如果最后最大的和还是负数的话，则输出 0 以及数据的首尾两个元素。
...
这道题目上面一道题目是差不多的。状态转移方程为：$$f[i] = \max{\left(f[i - 1] + pData[i], pData[i]\right)}$$ 唯一的一个区别是状态转移的时候并不能严格大于，因为它要找到第一个，因此左端点一定是越小越好。
...
我们不妨考虑左边的情况，用 $pLeft[i]$ 表示第 $i$ 个矩形左边连续比它高的矩形最左边的矩形的编号。那么如果我们有另外一个矩形 $j$，存在 $pData[j] < pData[i]$，我们就可以将 $pLeft[j]$ 置为 $pLeft[i]$，然后再往左迭代。需要注意的是，左右两边需要分开进行。因为从左往右只能保证 $pLeft$ 数组被更新了，可以进行迭代，而 $pRight$ 则还是类似之前的暴搜一样，复杂度很高，因此我们需要将左右两边分开迭代。
...
关键问题在于如何在最短的时间内构造出这样的一个数组 $pData$，使得 $pData[i][j]$ 表示从第 $i$ 行第 $j$ 个向上搜索有几个连续的 'F'。

我们可以考虑一个类似前缀和的思想，如果当前读入的地形为 'F'，那么 $pData[i][j] = pData[i - 1][j] + 1$，表示和上面组成一个更大的矩形；否则，$pData[i][j] = 0$，表示不能和上面的组成更大的矩形。
...
简单的 0/1 背包问题，状态转移方程：$$f[j] = \max{\left(f[j - pCost[i]] + pValue[i], f[j]\right)}
...
这道题目是求上升序列的最大和。用 $f[i]$ 表示以第 $i$ 个节点结尾可以达到的最大和，那讲最长上升序列改造一下即可，状态转移方程基本不变：$$f[i] = \max{f[j]} + pData[i]$$ 其中 $1\leq j < i$。
...
最后，状态转移方程为：$$ f[i][j] = \max{\left(f[i - 1][j], f[i][j - 1], f[i][j / k]\right)} + pData[i][j]$$ 其中 $f[i][j]$ 表示走到点 $(i, j)$ 能够获得的最大幸运值，同时 $k$ 是 $j$ 的约数。
...
我们用 $f[i]$ 表示以第 $i$ 个方块为最后一个方块所能到达的最大高度。那么状态转移方程为：$$f[i] = \max{f[j]} + pData[i].z$$ 其中 $pData[i].z$ 表示第 $i$ 个方块的高度，且满足 $1\leq j < i$。

此外转移的条件为可以将第 $i$ 个方块放到第 $j$ 个方块上面，或者第 $i$ 个方块可以放到第 $j$ 个方块的下面，即 $pData[i].x < pData[j].x \&\& pData[i].y < pData[j].y || pData[i].x > pData[i].x \&\& pData[i].y >pData[i].y$。

最后的答案就是 $\max{pData[i].z}$。
...
状态转移方程为：$$f[j] = 1 iff f[j - pCost[i]] = 1$$

最后我们只要计算小于等于 $V / 2$ 的情况下最多能够装多少体积的物品，记作 $ans$。那么答案就是 $V - ans$ 以及 $ans$。
...
当然，这里还需要处理一下边界 0 和 10 的情况，具体表现为：

* $f[i][0] += \max{\left(f[i + 1][0], f[i + 1][1]\right)}$
* $f[i][10] += \max{\left(f[i + 1][9], f[i + 1][10]\right)}$
...
与之不同的是，我们在这里不需要考虑转移的条件，直接置所有的 $f[j] = 1$。而状态转移方程则为：$$f[j] = \min{\left(f[j - pCost[i]] * pValue[i], f[j]\right)}$$ 最后输出的时候注意一下格式即可。
...
我们使用f[j][k]表示杀了j个怪兽，使用了k个忍耐值，能够获得的最大经验。

那么状态转移方程为：f[j][k] = max(f[j - 1][k - pBare[i]] + pExp[i], f[j][k])。

最后，若f[S][M] < N，则无解。否则我们需要求解能够升级的情况下所使用的最少的忍耐值nTmp，输出M - nTmp即可。
...
我们令f[i], g[i]分别表示输入完i个字符后，Caps Lock灯关与开的状态下所需要的按键次数，易得状态转移方程：

第i个字母小写：f[i] = min(f[i - 1] + 1, g[i - 1] + 2), g[i] = min(f[i - 1] + 2, g[i - 1] + 2)；
第i个字母大写：f[i] = min(f[i - 1] + 2, g[i - 1] + 2), g[i] = min(f[i - 1] + 2, g[i - 1] + 1)。
下面我们来分析一下上面的状态转移方程：

第i个字母小写，要保持最后Caps Lock灯关，那么在灯关的状态下直接输入字母，因此为f[i - 1] + 1；在灯开的状态下关灯，输入字母，因此为g[i - 1] + 2。如果要保持最后Caps Lock灯开，在灯关的状态下输入字母，开灯，因此为f[i - 1] + 2；在灯开的状态下Shift + 字母，因此为g[i - 1] + 2。当第i个字母大写的情况类似。

初始条件如下：

第1个字母小写：f[0] = 1, g[0] = 2；
第1个字幕大写：f[0] = 2, g[0] = 2。
最后答案为min(f[x.length() - 1], g[x.length() - 1] + 1)，因为最后Caps Lock灯要保持关闭状态。
...
一个很明显的思路是构造Ci个物品Ai，然后做一遍0-1背包即可。但是会TLE，因此需要寻求优化的方法。

在这里我们采用二进制拆分的方法。对于Ci个物品Ai，将它拆分成Ai * (1 + 2 + 4 + 8 + 16 + ... + left)，根据二进制的知识，我们知道通过这样的方法可以获得 1 * Ai, 2 * Ai, .. Ci * Ai的各个数值。证明如下：

这就等效于证明可以从1, 2, 4, 8, 16, ..., left中选取任意个数字组成1, 2, 3, ..., Ci。我们将这些数字写成二进制的形式，会发现这与前面的拆分是一一对应的。因此这样拆分物品不影响结果的正确性，接下来做一遍0-1背包即可出解。
